 ...
 */
public class ParseSignupFragment extends ParseLoginFragmentBase implements OnClickListener, BiometricAuthListener {
  ...
  public void onClick(View v) {
    ...
    if (username.length() == 0) {
      ...
      if (name != null && name.length() != 0 && config.isParseSignupNameFieldEnabled()) {
        user.put(USER_OBJECT_NAME_FIELD, name);
      }

      if (BiometricUtil.isBiometricReady(this.getContext())) {
  ...
  private void signupSuccess(@NonNull BiometricPrompt.AuthenticationResult result) {
    Cipher cipher = result.getCryptoObject().getCipher();
    Credential credential = new Credential(user.getUsername(), passwordField.getText().toString());
  ...
  public void onBiometricAuthenticationError(int errorCode, @NonNull String errorMessage) {
    CommonUtils.displayToast(this.getContext(), "Biometric error: $errorMessage");
  }
