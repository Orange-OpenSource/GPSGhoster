...
object DataBaseUtils {
    ...
    fun reset(context: Context, databaseName : String, databasePassword : String) {
        if (BuildConfig.DEBUG) {
            try {
                val debugDB = Class.forName("com.amitshekhar.DebugDB")
                val shutDown = debugDB.getMethod("shutDown")
                shutDown.invoke(null)

                val argTypes = arrayOf<Class<*>>(Context::class.java, DBFactory::class.java)
                val initialize = debugDB.getMethod("initialize", *argTypes)
                initialize.invoke(null, context, DebugDBEncryptFactory())

                setCustomDatabaseFiles(context, databaseName, databasePassword)
            } catch (exception: Exception) {
            }
        }
    }
